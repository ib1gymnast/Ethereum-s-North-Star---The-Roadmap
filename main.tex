...
\begin{wrapfigure}{r}{0.45\textwidth}
   ...
   \caption*{\href{https://www.strategicethreserve.xyz/}{SΞR}}
\end{wrapfigure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{eth issuance.png}
...
By the end of this paper, you will understand what each layer is responsible for and the upgrades that are coming for all three. 
...
The end result is that economic finality is achieved far faster with an Orbit mechanism than 64 slots as in today's finality mechanism. 
...
Personally, I think there's a decent chance we don't see an implementation of SSLE in Ethereum until after zero-knowledge proofs become core to the protocol. 
...
As mentioned above, L2 networks must post their data to Etheruem L1 such that it is available for anyone to verify correct transaction execution. 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{blob costs.PNG}
...
One way we can scale this data availability throughput further, without incresaing validator hardware requirements, is through \textbf{Data Availability Sampling.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{erc20 data compression.jpg}
    \caption{\textit{reducing erc20 transfer data requirements, original 
...
The rest of the Surge track is a bit beyond the scope of this paper, but you can read more about it in Vitalik's \href{https://vitalik.eth.limo/general/2024/10/17/futures2.html}{blog post.} 
...
\subsubsection*{\normalsize \quad Making state verification and execution easy}
...
\begin{itemize}
    \item \textbf{Stateless Verification:} Minimizing the amount of data that is needed to store the \textit{current state} of the blockchain $S$ (which today consumes hundreds of GB (input correct stat here) and needs to be stored by fully verifying Ethereum nodes
...
Rollups that take advantage of this new precompile will be called \textit{Native Rollups}. Native Rollups have significant advantages over non-native rollups, namely around reducing attack vectors for users, simplicity of development, and forward compatibility with Ethereum L1. \href{https://x.com/pumatheuma}{Uma Roy}, CEO of \href{https://x.com/SuccinctLabs}{Succint Labs} developing a leading zkVM, and Justin Drake do a fantastic job \href{https://youtu.be/0TOnHHnB6Ro?si=dZFF5jDdiLCUE4ou}{breaking this down} in a Bankless interview. \\
...
In the Bankless interview with Uma and Justin, David does a great job \href{https://youtu.be/0TOnHHnB6Ro?si=7UiKfIUPfdSi_Wuk&t=1827}{revisiting} the concept of Rollup \hyperref[Stages]{stages}, in which there are five requirements for rollups. One of those requirements involves having \href{https://l2beat.com/scaling/projects/arbitrum#state-validation}{very complex} fraud proof infrastructure that would allow a user to prove a faulty state transition occurred and revert it. \begin{wrapfigure}{r}{0.3\textwidth}
   ...
   \caption*{\textit{State Validation is removed as a risk vector for users, graphic from \href{https://l2beat.com/scaling/projects/arbitrum}{L2Beat}}}
...
\begin{figure}[H]
    ...
    \caption{\textit{A (not exhaustive) Distribution of MEV bot profits, data from \href{https://libmev.com/leaderboard}{libmev.com} by \href{https://x.com/crypticwoods_}{@cripticwoods\_}}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{MEV-Boost share, by \href{https://x.com/Data_Always}{@Data\_Always}}}
\end{figure}
...
According to soispoke, for an IL committee of size 16, statistically only about 15\% of the validator set need to be honest for transactions to be included within the same slot. In this simulation, FOCIL seems to be a fairly robust mechanism for ensuring censorship resistance. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{mechanstein.jpeg}
...
The reason is because BuilderNet will \textit{attract orderflow from users}, increasing MEV opportunities fo block builders, by issuing  \href{https://buildernet.org/docs/refunds}{\textit{user refunds}}, enabled by a \href{https://www.youtube.com/watch?v=VUN-k_nfDts}{\textit{second price auction}} mechanism. Through this refund mechanism, BuilderNet will be the best place for users to go who want to land a transaction onchain. 

Second price auctions work by having the winner of a bid pay the amount of the second highest bid, which was by definition a lower amount than their bid, which was the highest. Second price auctions incentivize users to pay much closer to the true value for a transaction they want to land onchain. The refunds are distributed back to the users who generated the orderflow based on the MEV value they contributed to the block. 

This refund mechanism will attract infrastructure providers (who want to attract users), allowing BuilderNet to create a network effect around attracting the most valuable orderflow. 
...
\begin{align}
    APR = \frac{2.6*64}{\sqrt{total\ staked\ ETH}}
\end{align}

An important concept is \textit{real yield}, which is the difference between the total issuance of the network and the expected yield for each validator. As an example, if each validator earned 2\% staking yield but the network issuance was 2\% per year, then the \textit{real yield would be zero.}
...
\begin{figure}[H]
    ...
    \caption{\textit{As more ETH is staked, real yield (dashed green line) tends to near zero, even though nominal yield remains close to 2\%}}
\end{figure}
...
As more ETH is staked, the network's aggregate issuance increases, which has the effect of increasing dilution for un-staked ETH holders. This \textit{increases the incentive} for un-staked ETH holders to stake their ETH \textit{just to avoid the effects of dilution.} With staked ETH ETFs \href{https://x.com/Grayscale/status/1975147923387363421}{coming online}, staking ETH is about to become even more frictionless. \href{https://x.com/JustDeauIt}{Mike Nadeau} wrote an \href{https://thedefireport.io/research/the-50-billion-cascade#impact-on-economic-security}{excellent article} discussing the impacts of staked ETH ETFs. As staking friction decreases, more ETH will be staked, further diluting un-staked ETH holders. What we have is a tragedy of the commons. 

One method of staking for Ethereum users is through Liquid Staking Tokens, such as stETH, 

\href{https://youtu.be/m91Wu6-cdwk?si=pEZ6Y7L0K3Wq7ZQa}{Anders' talk}

- tragedy of the commons in the higher staking regime, as people will stake to avoid dilution, while still missing out on any real yield
- one of the goals is that holding raw eth should be economically viable
- why is high issuance centralizing?
    - volatility of MEV rewards
    - economies of scale
- how do we target a specific stake participation ratio? (define this as well)
- introduce dis-economies of scale, maybe a combination of this and targeted stake participation would work well in tandem

\subsubsection{Making Ethereum Quantum Resistant}

\begin{aside}
    \textit{A Note on Ethereum's Quantum Resistance:} Ethereum is also deeply considering hardening itself against quantum threats, but it is early enough in exploring this that it would be premature for me to dedicate a full section to it. 
\end{aside}
...
The Protocol Guild is an independent funding organization that helps to fund the work of 190 open source Ethereum core developers. \\

This Splits contracts I made are immutable, meaning they can never change, so you know exactly where your donated money will go and you can verify that onchain. You can verify The Protocol Guild's donation addresses \href{https://protocol-guild.readthedocs.io/en/latest/03-donate.html}{here}. \\

To donate to me, donate to any of the Splits addresses below: 
...
In lieu of a conclusion, I'll summarize the paper in a one page document on the next page. At this point, hopefully you understand everything below. If you don't, feel free to reach out with some questions!

One page summary is on the following page:

Twitter \href{https://twitter.com/i/spaces/1OyKAjlLgdMGb}{space} on post-quantum cryptography

Youtube series on \href{https://www.youtube.com/watch?v=7it17ANqdBE&list=PLJqWcTqh_zKGPctGzVOBllZnCPQj-g7d3&index=5}{Post Quantum Security}

\clearpage

\subsection*{One Pager:}

\subsubsection*{Ethereum's Network Effects Are Strengthening:}

\begin{itemize}
    \item \textbf{Stablecoin Supply} on Ethereum \href{https://defillama.com/stablecoins/ethereum}{is up} 46.8\% in 2025 to \$160B; Bessent believes we may \href{https://x.com/SecScottBessent/status/1935027160374210573}{see \$3.7 Trillion onchain} by 2030
    \item \textbf{Transactions Per Second} in the Ethereum ecosystem is on a \href{https://l2beat.com/scaling/activity}{strong uptrend}, up 5x since early 2024
    \item \textbf{ETH as a Store of Value:} ETH DATs and ETFs \href{https://l2beat.com/scaling/activity}{have acquired} 7.8M Ether since the beginning of June
\end{itemize}

\subsubsection*{Ethereum's Goal}

Ethereum's North Star is to be the Internet's most credibly neutral and censorship resistant financial layer, such that it becomes the global financial center of gravity. This is enabled by the following upgrades to Ethereum's three layers:

\subsubsection*{Consensus Layer}

\begin{itemize}
    \item time to finality will decrease $\approx$ 32x, via \hyperref[Single Slot Finality]{Single Slot Finality}
    \item \hyperref[Preconfirmations]{Preconfirmations} will create a fintech user experience on Ethereum, enabling almost instant transaction confirmations for users
    \item increased resilience to Denial-of-Service attacks via \hyperref[Single Secret Leader Election]{Single Secret Leader Election}
\end{itemize}

\subsubsection*{Data Layer}

\begin{itemize}
    \item \hyperref[PeerDAS]{Data Availability Sampling} will allow the network to load-share Layer 2 transaction data across the network, linearly increasing scalability with each new Ethereum validator added to the network
    \item transactions per second on Layer 2s will allow up to $\approx$ 10M over the next 3 - 5 years as the blob target increases from 6 to 128 per block
    \item Based and Native Rollups note
\end{itemize}

\subsubsection*{Execution Layer}

\begin{itemize}
    \item Validity proofs will be constant time to verify block correctness, enabling a vastly increased block gas limit and 10k TPS on Ethereum Layer 1
    \item stateless verification will reduce storage requirements, in combination with validity proofs, \textit{validating on smart watches will be possible}
    \item native execution will enable trustless rollups, deprecation of governance vulnerabilities, and 
\end{itemize}

\subsubsection*{Entities building on Ethereum:}

\href{https://www.youtube.com/live/FBHmAq5lmZQ?si=Fy8OwPsKn4S-HTef}{Robinhood} and \href{https://www.investing.com/news/company-news/etoro-expands-245-trading-plans-tokenized-stocks-on-ethereum-93CH-4157860#:~:text=The%20company%20also%20announced%20plans%20to%20launch%20U.S.%2Dlisted%20equities%20as%20ERC20%20tokens%20on%20the%20Ethereum%20blockchain}{eToro} are tokenizing stocks, Visa + PayPal, SWIFT is building on Linea, an Ethereum Layer 2. \href{https://www.coinbase.com/blog/Coinbase-and-JPMorgan-Chase-join-forces-to-make-it-even-easier-to-access-crypto}{JPMorgan + Coinbase}, 
